...
/*CUSTOM FUNCTIONS added by Mattio-cmd*/
/*
void autostart(void) {
    if (fork() == 0) {
        setsid();
        execlp("$HOME/.config/dwm-6.5/autostart.sh", "$HOME/.config/dwm-6.5/autostart.sh", NULL);
        exit(EXIT_SUCCESS);
    }
}
*/
...
tile(Monitor *m)
{
	...
	for (i = 0, my = ty = m->gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
      ...
      resize(c, m->wx + m->gappx, m->wy + my, mw - (2*c->bw) - m->gappx, h - (2*c->bw), c->bw, 0);
			if (my + HEIGHT(c) + m->gappx < m->wh)
				my += HEIGHT(c) + m->gappx;
		} else {
			h = (m->wh - ty) / (n - i) - m->gappx;
      resize(c, m->wx + mw + m->gappx, m->wy + ty, m->ww - mw - (2*c->bw) - 2*m->gappx, h - (2*c->bw), c->bw, 0);
...
//  autostart(); // <-- CUSTOM FUNCTION added by Mattio-cmd
